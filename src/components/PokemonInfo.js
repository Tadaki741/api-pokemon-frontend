    ...
    <div className="relative border rounded-lg text-center cursor-pointer transition hover:border-x-cyan-200 hover:border-y-purple-200 hover:shadow-lg">
      ...
      <div className="border-t flex flex-row">
        <button
          ...
          onClick={() => {
            updateQuantity(pokemon.id, pokemon.quantity + 1);
          }}
        >
      ...
      {showDetail === pokemon.id ? (
        ...
        <div className="absolute top-0 left-0 rounded-lg w-full h-full backdrop-blur-md flex flex-col justify-center items-center">
          ...
          <button
            className="flex items-center justify-center rounded-lg bg-rose-600 text-white font-bold px-4 transition hover:bg-rose-700 mb-3"
            disabled={mutationUpdatePokemon.isLoading}
            onClick={() => {
              updateQuantity(pokemon.id, 0);
            }}
          >
            <ExclamationIcon />
            <span>Reset</span>
          ...
          <div className="w-full border-t">
            <button
              className="flex items-center justify-center rounded-lg bg-yellow-200 text-white font-bold px-4 transition hover:bg-yellow-300 mx-auto mb-3"
              disabled={mutationUpdatePokemon.isLoading}
              onClick={() => {
                handleDelete(pokemon.id, 0);
              }}
            >
              <TrashIcon></TrashIcon>
              <span>Delete</span>
